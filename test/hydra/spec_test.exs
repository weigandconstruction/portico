defmodule Hydra.SpecTest do
  ...
  describe "parse/1" do
    ...
    test "parses a complete OpenAPI spec with all fields" do
      ...
      assert spec.components == %{"schemas" => %{"User" => %{"type" => "object", "properties" => %{"id" => %{"type" => "integer"}, "name" => %{"type" => "string"}}}}}
      assert spec.security == [%{"bearerAuth" => []}]
      assert spec.tags == [%{"name" => "users", "description" => "User operations"}]
      assert spec.external_docs == %{"description" => "Find more info here", "url" => "https://example.com"}
    end
