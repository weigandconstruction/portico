defmodule Portico.TemplateRenderingTest do
  ...
  describe "generated documentation" do
    ...
    test "stamps moduledocs with generation date and Portico reference", %{
      temp_dir: temp_dir
    } do
      spec =
        create_test_spec([
          create_path("/users", [
            create_operation("get", "users", "List all users")
          ])
        ])

      generate_and_test(spec, temp_dir, "TestAPI")

      # The date is the local date at generation time, so assert the full
      # reference sentence shape (link + ISO date) rather than an exact date.
      portico_reference =
        ~r/Code generated by \[Portico\]\(https:\/\/github\.com\/weigandconstruction\/portico\) from an OpenAPI 3\.0 specification on [0-9]{4}-[0-9]{2}-[0-9]{2}\./

      client_content =
        File.read!(Elixir.Path.join(temp_dir, "lib/test_api/client.ex"))

      api_content = File.read!(Elixir.Path.join(temp_dir, "lib/test_api/api/users.ex"))

      assert client_content =~ "@moduledoc"
      assert client_content =~ portico_reference
      assert api_content =~ portico_reference
    end
  end
