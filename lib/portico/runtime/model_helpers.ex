defmodule Portico.Runtime.ModelHelpers do
  ...
  @doc """
  Normalizes incoming JSON parameters so they can be fed to `Ecto.Changeset.cast/3`.
  ...
  params map, so we normalize to all-strings: renamed keys are
  rewritten, already-matching keys pass through unchanged, atom keys
  are stringified.

  The previous implementation used `String.to_existing_atom/1` and
  silently dropped any key that didn't already exist as an atom — which
  meant every renamed camelCase key was lost without warning. This
  version never drops data.
  """
